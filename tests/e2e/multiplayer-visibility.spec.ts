...
async function setupPlayer(browser: Browser, playerName: string): Promise<TestPlayer> {
  ...
  page.on('console', msg => {
    ...
    if (text.includes('[ERROR]') || text.includes('[DEBUG]') || text.includes('Player')) {
...
async function getVisibleEntities(page: Page): Promise<Array<{ id: string; type: string; position: number[] }>> {
  return await page.evaluate(() => {
    const world = (window as any).world;
...
async function getMinimapPips(page: Page): Promise<number> {
  return await page.evaluate(() => {
    ...
    const minimapPips = document.querySelectorAll('[data-testid*="minimap-pip"]');
...
test.describe('Multiplayer Visibility', () => {
  test('two players should see each other', async ({ browser }) => {
    ...
    try {
      ...
      console.log('Player 1 new position (as seen by Player 2):', p1NewPosFromP2);
