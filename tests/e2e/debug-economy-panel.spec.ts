...
test.describe('Debug Economy Panel', () => {
  ...
  test('debug panel should be visible when F9 is pressed', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="debug-spawn-item"]')).toBeVisible();
    await expect(page.locator('[data-testid="debug-spawn-item-2"]')).toBeVisible();
    await expect(page.locator('[data-testid="debug-add-gold"]')).toBeVisible();
    await expect(page.locator('[data-testid="debug-trigger-death"]')).toBeVisible();
    await expect(page.locator('[data-testid="debug-initiate-trade"]')).toBeVisible();
  });
  ...
  test('trigger death button should kill player and drop items', async ({ page }) => {
