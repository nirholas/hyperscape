...
describe("Hyperscape Event Publishing", () => {
  ...
  beforeAll(async () => {
    ...
    if (!hyperscapeOracleAddress) {
      console.warn(
        "⚠️  HYPERSCAPE_ORACLE_ADDRESS not set - test will be skipped",
      );
      console.warn(
        "   Deploy contracts first: cd contracts && forge script script/DeployPredictionMarkets.s.sol --broadcast",
      );
      return;
    }
  ...
  test("should capture SkillLevelUp events", async () => {
    ...
    console.log(
      `📊 Found ${events.length} SkillLevelUp events in last 1000 blocks`,
    );
  ...
  test("should capture PlayerDeath events", async () => {
    ...
    console.log(
      `📊 Found ${events.length} PlayerDeath events in last 1000 blocks`,
    );
  ...
  test("should capture PlayerKill events", async () => {
    ...
    console.log(
      `📊 Found ${events.length} PlayerKill events in last 1000 blocks`,
    );
...
console.log(
  "   forge script script/DeployPredictionMarkets.s.sol --rpc-url http://localhost:8545 --broadcast --private-key 0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80",
);
console.log("");
