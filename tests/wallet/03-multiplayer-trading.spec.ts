...
test.describe('Hyperscape - Multi-Player Trading System', () => {
  ...
  test('should initiate trade request programmatically', async ({ page, browser }) => {
    ...
    console.log(`Trade request received by Player 2: ${receivedTradeRequest ? '✅' : '⚠️  (may need UI to verify)'}`)
    ...
    const validationTests = await page.evaluate(() => {
      ...
      try {
        if (world?.network?.socket?.send) {
          ...
          world.network.socket.send('tradeConfirm', { tradeId: 'fake-id-12345' })
