...
test.describe('Hyperscape - Multiplayer Visibility', () => {
  ...
  test('TWO PLAYERS MUST SEE EACH OTHER OR TEST CRASHES', async ({ context, page }) => {
    ...
    if (p1Count !== 2) {
      ...
      throw new Error(`MULTIPLAYER BROKEN: Player 1 sees ${p1Count} players instead of 2`)
    }
    ...
    if (p2Count !== 2) {
      ...
      console.error(`This means multiplayer visibility is BROKEN!`)
      console.error(`Check screenshots in test-results/`)
      throw new Error(`MULTIPLAYER BROKEN: Player 2 sees ${p2Count} players instead of 2`)
    }
    ...
    if (!p1SeeIds.includes(player2Id!)) {
      throw new Error(`Player 1 doesn't see Player 2's entity! Saw IDs: ${p1SeeIds.join(', ')}`)
    }
    
    if (!p2SeeIds.includes(player1Id!)) {
      throw new Error(`Player 2 doesn't see Player 1's entity! Saw IDs: ${p2SeeIds.join(', ')}`)
    }
