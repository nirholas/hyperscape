...
test.describe('Hyperscape Economy - Complete Testing', () => {
  ...
  test('02 - Spawn item and verify it appears in inventory', async ({ page }) => {
    // Open debug panel
    await page.keyboard.press('F9')
    await page.waitForTimeout(500)
    ...
    const slotsBefore = await page.locator('[data-testid^="inventory-slot"]').count()
  ...
  test('03 - Verify MintItemButton appears and is clickable', async ({ page, context, metamaskPage, extensionId }) => {
    ...
    const mintButton = page.locator('button').filter({ hasText: /mint|temporary/i }).first()
  ...
  test('04 - Complete minting flow with MetaMask signature', async ({ page, context, metamaskPage, extensionId }) => {
    const metamask = new MetaMask(context, metamaskPage, basicSetup.walletPassword, extensionId)
  ...
  test('05 - Death drops: minted items stay, unminted items drop', async ({ page }) => {
    // Spawn two items
    await page.keyboard.press('F9')
    await page.waitForTimeout(500)
    ...
    const itemsBefore = await page.locator('[data-testid^="inventory-slot"]').count()
    ...
    const itemsAfter = await page.locator('[data-testid^="inventory-slot"]').count()
  ...
  test('07 - Verify GoldClaimButton exists and is functional', async ({ page, context, metamaskPage, extensionId }) => {
    const metamask = new MetaMask(context, metamaskPage, basicSetup.walletPassword, extensionId)
    ...
    const claimButton = page.locator('button').filter({ hasText: /claim.*gold/i }).first()
    ...
    if (claimButtonExists > 0) {
      ...
      console.log('⚠ GoldClaimButton not found in UI (may need to be added to visible area)')
    }
  })
  ...
  test('09 - Complete economy cycle end-to-end', async ({ page, context, metamaskPage, extensionId }) => {
    const metamask = new MetaMask(context, metamaskPage, basicSetup.walletPassword, extensionId)
    ...
    await page.keyboard.press('i')
    await page.waitForTimeout(1000)
    const slotsAfter = await page.locator('[data-testid^="inventory-slot"]').count()
