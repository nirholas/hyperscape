...
const worldsData = JSON.parse(
  readFileSync("../contracts-mud/mmo/worlds.json", "utf-8"),
);
const WORLD_ADDRESS = (worldsData["31337"]?.address ||
...
console.log(
  `${colors.gray}Using world address: ${WORLD_ADDRESS}${colors.reset}`,
);
...
async function test(name: string, fn: () => Promise<void>) {
  ...
  try {
    ...
    console.log(
      `   ${colors.gray}${error instanceof Error ? error.message : error}${colors.reset}`,
    );
    failed++;
  }
}
...
async function main() {
  ...
  console.log(
    `${colors.gray}Testing with account: ${account.address}${colors.reset}\n`,
  );
  ...
  await test("Items created on-chain", async () => {
    ...
    if (itemName !== "Logs")
      throw new Error(`Expected "Logs", got "${itemName}"`);
  });
  ...
  if (failed === 0) {
    ...
    console.log(
      "║   ✅ ALL TESTS PASSED - BLOCKCHAIN FULLY FUNCTIONAL          ║",
    );
    console.log(
      "║                                                              ║",
    );
    ...
    console.log(
      `║   ❌ ${failed} TEST(S) FAILED - BLOCKCHAIN NOT READY              ║`,
    );
    console.log(
      "║                                                              ║",
    );
