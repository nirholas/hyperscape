...
for (const dep of mudDeps) {
  if (
    clientPackage.dependencies?.[dep] ||
...
const blockchainGatewayExists = await Bun.file(
  "packages/server/src/BlockchainGateway.ts",
).exists();
const databaseSystemFile = await Bun.file(
...
const hasGatewayInDatabase =
  databaseSystemFile.includes("blockchainGateway") &&
...
const testFile = await Bun.file(
  "packages/plugin-hyperscape/tests/mud-integration.test.ts",
).text();

const skippedTests = (testFile.match(/test\.skip/g) || []).length;
const activeTests = (
  testFile.match(/test\('should register a player on-chain'/g) || []
).length;
...
const startLocalnetScript = await Bun.file(
  "scripts/start-localnet.ts",
).exists();
const startFullScript = await Bun.file(
  "../../scripts/start-hyperscape-full.sh",
).exists();
...
if (
  envExample.includes("WORLD_ADDRESS") &&
  envExample.includes("ANVIL_RPC_URL")
) {
  check(
    "Environment vars for blockchain",
    "pass",
    "Found WORLD_ADDRESS and RPC configuration",
  );
} else {
  check(
    "Blockchain env vars MISSING",
    "fail",
...
if (failed === 0 && warnings === 0) {
  ...
  console.log(
    `${colors.red}CRITICAL: Game does not use blockchain${colors.reset}`,
  );
  console.log(
    `${colors.gray}See ONCHAIN_ASSESSMENT.md for detailed analysis${colors.reset}\n`,
  );
  process.exit(1);
} else {
  console.log(
  ...
  console.log(
    `${colors.yellow}WARNING: Some blockchain components missing${colors.reset}\n`,
  );
  process.exit(1);
}
