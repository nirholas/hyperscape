...
try {
  ...
  const anvilProcess = spawn('anvil', [
    '--block-time', '2',  // 2 second blocks (like Jeju L3)
    '--port', '8545',
  ...
  console.log(`${colors.gray}   Waiting for anvil to initialize...${colors.reset}`);
  ...
  try {
    ...
    if (response.ok) {
      console.log(`${colors.green}✅ Anvil started successfully${colors.reset}`);
...
if (!existsSync(worldsJsonPath)) {
  console.error(`${colors.red}❌ Deployment file not found: ${worldsJsonPath}${colors.reset}`);
  console.error('   Contracts may not have deployed correctly');
  process.exit(1);
}

const deploymentData = JSON.parse(readFileSync(worldsJsonPath, 'utf-8'));
const worldAddress = deploymentData['31337']?.address || deploymentData.worldAddress;

if (!worldAddress) {
  console.error(`${colors.red}❌ World address not found in deployment data${colors.reset}`);
  process.exit(1);
}
...
try {
  ...
  console.warn(`${colors.yellow}⚠️  World initialization may have already been done${colors.reset}\n`);
}
...
try {
  ...
  if (initialized) {
    ...
    console.log(`${colors.yellow}⚠️  World not initialized (will initialize on first use)${colors.reset}`);
  }
  ...
  if (itemOutput.includes('Logs')) {
    console.log(`${colors.green}✅ Items created (sample: Logs)${colors.reset}`);
  }
  
} catch (error) {
  console.warn(`${colors.yellow}⚠️  Could not verify deployment (this is okay)${colors.reset}`);
}
...
console.log('╚══════════════════════════════════════════════════════════════╝');
console.log('');
console.log(`${colors.gray}Configuration saved to packages/*/. env.local${colors.reset}`);
console.log('');
console.log('🎮 Next steps:');
console.log(`${colors.gray}   1. Start the game server: ${colors.blue}npm start${colors.reset}`);
console.log(`${colors.gray}   2. Open browser: ${colors.blue}http://localhost:5555${colors.reset}`);
console.log(`${colors.gray}   3. Verify integration: ${colors.blue}bun scripts/verify-blockchain-integration.ts${colors.reset}`);
console.log('');
console.log('📊 Blockchain info:');
console.log(`${colors.gray}   RPC: http://localhost:8545${colors.reset}`);
console.log(`${colors.gray}   World: ${worldAddress}${colors.reset}`);
console.log(`${colors.gray}   Account: 0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266${colors.reset}`);
console.log('');
