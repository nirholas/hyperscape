...
- Inventory slots free: ${possibilitiesData.inventorySlotsFree}`;
   ...
   Steps: ${g.steps.slice(0, 2).join(", ")}...`,
      ...
      // Build intelligent LLM prompt with full context
...
Choose the goal ID that makes the most sense. Respond with ONLY the goal ID (e.g., "woodcutting_basics" or "combat_training_goblins"). Nothing else.`;
      ...
      try {
        ...
        // Fallback to highest scored goal if LLM fails
        logger.warn(
          `[SET_GOAL] LLM selection failed, using highest scored: ${llmError}`,
        );
        selectedGoalId = goalTemplates[0].id;
        selectionMethod = "fallback (highest score)";
      }

      // Find the selected goal template
      let selectedGoal = goalTemplates.find((g) => g.id === selectedGoalId);
      if (!selectedGoal) {
        // Try partial match
        selectedGoal = goalTemplates.find(
          (g) => g.id.includes(selectedGoalId) || selectedGoalId.includes(g.id),
        );
        if (!selectedGoal) {
          // Fallback to highest scored if invalid selection
          logger.warn(
            `[SET_GOAL] Invalid goal ID "${selectedGoalId}", using highest scored`,
          );
          selectedGoal = goalTemplates[0];
