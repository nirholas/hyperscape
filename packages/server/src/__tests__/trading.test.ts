...
describe('Trading System', () => {
  ...
  beforeEach(() => {
    ...
    network = new ServerNetwork(mockWorld as unknown as Parameters<typeof ServerNetwork>[0]);
  ...
  test('should initiate trade request', async () => {
    ...
    expect((tradeRequest!.data as { fromPlayerId: string }).fromPlayerId).toBe('player1');
    expect((tradeRequest!.data as { fromPlayerName: string }).fromPlayerName).toBe('Alice');
  });
  ...
  test('should reject trade if players too far apart', async () => {
    ...
    const originalPos = { ...(player2Socket.player as { position: { x: number; y: number; z: number } }).position };
    (player2Socket.player as { position: { x: number; y: number; z: number } }).position = { x: 100, y: 0, z: 100 };
    ...
    expect((error!.data as { message: string }).message).toContain('too far away');

    // Move player 2 back
    (player2Socket.player as { position: { x: number; y: number; z: number } }).position = originalPos;
  });
