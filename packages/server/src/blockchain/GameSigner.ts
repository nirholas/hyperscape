...
export class GameSigner {
  ...
  private mintCounts: Map<string, { count: number; resetAt: number }> =
    new Map();
  ...
  async signGoldClaim(request: GoldClaimRequest): Promise<GoldClaimSignature> {
    ...
    if (lastClaim && now - lastClaim < this.CLAIM_COOLDOWN_MS) {
      throw new Error(
        "[GameSigner] Claim cooldown not met. Try again in a few seconds.",
      );
    }
  ...
  async signItemMint(request: ItemMintRequest): Promise<ItemMintSignature> {
    ...
    if (this.mintedInstances.has(instanceId)) {
      throw new Error(
        "[GameSigner] Instance ID already minted. Possible duplication attack.",
      );
    }
    ...
    if (mintRecord) {
      if (now > mintRecord.resetAt) {
        // Reset window
        this.mintCounts.set(playerKey, {
          count: 1,
          resetAt: now + this.MINT_RATE_WINDOW_MS,
        });
      } else if (mintRecord.count >= this.MINT_RATE_LIMIT) {
        throw new Error(
          "[GameSigner] Mint rate limit exceeded. Try again later.",
        );
      } else {
        mintRecord.count++;
      }
  ...
  clearMintedInstance(instanceId: string): void {
    ...
    console.log(
      "[GameSigner] Cleared minted instance:",
      instanceId.substring(0, 20) + "...",
    );
  }
...
export function initializeGameSigner(privateKey: string): GameSigner {
  if (gameSignerInstance) {
    console.warn(
      "[GameSigner] Already initialized, returning existing instance",
    );
...
export function getGameSigner(): GameSigner {
  if (!gameSignerInstance) {
    throw new Error(
      "[GameSigner] Not initialized. Call initializeGameSigner() first.",
    );
  }
  return gameSignerInstance;
}
