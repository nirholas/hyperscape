...
export class OraclePublisher {
  ...
  constructor(config: OraclePublisherConfig) {
    ...
    if (!this.enabled) {
      console.log(
        "[OraclePublisher] Disabled - Hyperscape events will not be published to blockchain",
      );
      return;
    }
    ...
    if (!config.hyperscapeOracleAddress) {
      console.warn(
        "[OraclePublisher] No oracle address - disabling event publishing",
      );
      this.enabled = false;
      return;
    }
    ...
    console.log(
      `[OraclePublisher]    Oracle: ${config.hyperscapeOracleAddress}`,
    );
  }
  ...
  async publishSkillLevelUp(data: SkillLevelUpData): Promise<boolean> {
    ...
    console.log(
      `[OraclePublisher] Publishing skill level-up: ${data.player} → ${data.skillName} level ${data.newLevel}`,
    );
    ...
    await tx.wait();
    console.log(
      `[OraclePublisher] ✅ Skill event published (Tx: ${tx.hash.substring(0, 10)}...)`,
    );
  ...
  async publishPlayerDeath(data: PlayerDeathData): Promise<boolean> {
    ...
    console.log(
      `[OraclePublisher] Publishing death: ${data.player} killed by ${data.killer}`,
    );
  ...
  async publishPlayerKill(data: PlayerKillData): Promise<boolean> {
    ...
    console.log(
      `[OraclePublisher] Publishing kill: ${data.killer} → ${data.victim}`,
    );
  ...
  async publishAchievement(data: AchievementData): Promise<boolean> {
    ...
    console.log(
      `[OraclePublisher] Publishing achievement: ${data.achievementType} for ${data.player}`,
    );
