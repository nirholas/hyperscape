...
} from "@hyperscape/shared/blockchain/mud-client";
...
export class BlockchainGateway extends SystemBase {
  ...
  // Configuration
  private readonly BATCH_ENABLED =
    process.env.BATCH_INVENTORY_CHANGES === "true";
  private readonly BATCH_INTERVAL = parseInt(
  ...
  async init(): Promise<void> {
    ...
    if (!isMudClientAvailable()) {
      console.log(
        "[BlockchainGateway] ℹ️  Blockchain integration disabled (no WORLD_ADDRESS)",
      );
      console.log(
        "[BlockchainGateway] ℹ️  Set WORLD_ADDRESS to enable hybrid mode",
      );
    ...
    try {
      ...
      console.log(
        "[BlockchainGateway] 🔍 Auto-detecting network (Jeju preferred, Anvil fallback)",
      );
      ...
      console.log(
        `[BlockchainGateway]    Network: ${this.mudClient.chain.name} (Chain ID: ${this.mudClient.chain.id})`,
      );
      console.log(
        `[BlockchainGateway]    RPC: ${this.mudClient.chain.rpcUrls.default.http[0]}`,
      );
      console.log(
        `[BlockchainGateway]    World: ${this.mudClient.worldAddress}`,
      );
      console.log(
        `[BlockchainGateway]    Account: ${this.mudClient.account.address}`,
      );
      console.log(`[BlockchainGateway]    Block: ${blockNumber}`);
      console.log(
        `[BlockchainGateway]    Mode: HYBRID (critical state on-chain, performance off-chain)`,
      );
      ...
      console.error(
        "[BlockchainGateway] ❌ Failed to initialize MUD client:",
        error,
      );
      console.error(
        "[BlockchainGateway] ⚠️  Falling back to PostgreSQL-only mode",
      );
      console.error("[BlockchainGateway] 💡 Ensure blockchain is running:");
      console.error(
        "[BlockchainGateway]    - Jeju: bun run dev (from repo root)",
      );
  ...
  async registerPlayer(
    ...
    if (!this.isEnabled()) {
      console.log(
        "[BlockchainGateway] Skipping registerPlayer (blockchain disabled)",
      );
      return null;
    }

    console.log(
      `[BlockchainGateway] 📝 Registering player "${name}" on blockchain...`,
    );
  ...
  async addItem(
    ...
    // Execute immediately
    console.log(
      `[BlockchainGateway] 📦 Adding item ${itemId} (qty: ${quantity}) to ${playerAddress}`,
    );
    ...
    console.log(
      `[BlockchainGateway] ✅ Item added (tx: ${receipt.transactionHash})`,
    );
  ...
  async removeItem(
    ...
    console.log(
      `[BlockchainGateway] ✅ Item removed (tx: ${receipt.transactionHash})`,
    );
  ...
  async equipItem(inventorySlot: number): Promise<{ txHash: string }> {
    ...
    console.log(
      `[BlockchainGateway] ⚔️  Equipping item from slot ${inventorySlot}`,
    );

    const receipt =
      await this.mudClient!.EquipmentSystem.equipItem(inventorySlot);

    console.log(
      `[BlockchainGateway] ✅ Item equipped (tx: ${receipt.transactionHash})`,
    );
  ...
  async unequipItem(equipSlot: number): Promise<{ txHash: string }> {
    ...
    const receipt =
      await this.mudClient!.EquipmentSystem.unequipItem(equipSlot);

    console.log(
      `[BlockchainGateway] ✅ Item unequipped (tx: ${receipt.transactionHash})`,
    );
  ...
  async recordMobKill(mobId: Address): Promise<{ txHash: string }> {
    ...
    console.log(
      `[BlockchainGateway] ✅ Mob kill recorded (tx: ${receipt.transactionHash})`,
    );
    console.log(`[BlockchainGateway]    Gas: ${receipt.gasUsed}`);
  ...
  async recordResourceGathered(
    ...
    console.log(
      `[BlockchainGateway] ✅ Resource gathered (tx: ${receipt.transactionHash})`,
    );
  ...
  private queueInventoryOperation(op: PendingInventoryOp): void {
    ...
    console.log(
      `[BlockchainGateway] 📋 Queued inventory operation (${queue.length} pending)`,
    );
  }

  private startBatchProcessing(): void {
    console.log(
      `[BlockchainGateway] 🔄 Starting batch processing (interval: ${this.BATCH_INTERVAL}ms)`,
    );
  ...
  private async processBatchedOperations(): Promise<void> {
    ...
    for (const [
      playerAddress,
      operations,
    ] of this.pendingInventoryOps.entries()) {
  ...
  async getSyncStatus(): Promise<{
    ...
    const pendingOps = Array.from(this.pendingInventoryOps.values()).reduce(
