...
export class A2AServer {
  ...
  async registerRoutes(fastify: FastifyInstance): Promise<void> {
    ...
    fastify.get('/.well-known/agent-card.json', async (_req: FastifyRequest, reply: FastifyReply) => {
  ...
  async handleRequest(req: FastifyRequest, reply: FastifyReply): Promise<void> {
    ...
    if (!request.jsonrpc || request.jsonrpc !== '2.0') {
      return reply.send(this.createError(request.id ?? null, A2A_ERROR_CODES.INVALID_REQUEST, 'Invalid JSON-RPC version'));
    }

    if (!request.method) {
      return reply.send(this.createError(request.id ?? null, A2A_ERROR_CODES.INVALID_REQUEST, 'Missing method'));
    }

    // Route to handler
    if (request.method === 'message/send' || request.method === 'message/stream') {
      await this.handleMessageSend(request, reply, request.method === 'message/stream');
    } else {
      return reply.send(this.createError(request.id ?? null, A2A_ERROR_CODES.METHOD_NOT_FOUND, `Unknown method: ${request.method}`));
    }
  }

  private async handleMessageSend(request: JSONRPCRequest, reply: FastifyReply, streaming: boolean): Promise<void> {
    const params = request.params as { message?: A2AMessage };

    if (!params || !params.message) {
      return reply.send(this.createError(request.id ?? null, A2A_ERROR_CODES.INVALID_PARAMS, 'Missing message'));
    }
    ...
    if (!dataPart || !dataPart.data) {
      return reply.send(this.createError(request.id ?? null, A2A_ERROR_CODES.INVALID_PARAMS, 'Missing data part'));
    }
    ...
    try {
      ...
      return reply.send(this.createError(request.id ?? null, A2A_ERROR_CODES.INVALID_PARAMS, (error as Error).message));
    }

    // Prevent replay attacks
    if (this.seenMessageIds.has(message.messageId)) {
      return reply.send(this.createError(request.id ?? null, A2A_ERROR_CODES.INVALID_REQUEST, 'Duplicate message ID'));
    }
    ...
    if (!result.success) {
      return reply.send(this.createError(request.id ?? null, A2A_ERROR_CODES.INTERNAL_ERROR, result.message));
    }
    ...
    if (streaming) {
      ...
      reply.raw.write(`data: ${JSON.stringify(this.createSuccess(request.id ?? null, responseMessage))}\n\n`);
    } else {
      return reply.send(this.createSuccess(request.id ?? null, responseMessage));
    }
  }
  ...
  private async executeSkill(skillId: string, agentId: string, data: Record<string, unknown>): Promise<{ success: boolean; message: string; data?: Record<string, unknown> }> {
    ...
    switch (skillId) {
      case 'join-game': {
        const playerName = optionalString(data.playerName, `Agent_${agentId.slice(0, 8)}`);
      ...
      case 'get-status': {
        const player = rpg.getAllPlayers?.()?.find((p: { id: string }) => p.id === agentId);
      ...
      case 'move-to': {
        const x = firstDefined(data.x as number | undefined, data.targetX as number | undefined);
        const y = firstDefined(data.y as number | undefined, data.targetY as number | undefined);
        const z = firstDefined(data.z as number | undefined, data.targetZ as number | undefined);
      ...
      case 'gather-resource': {
        ...
        const result = await this.world.actionRegistry?.execute('start_gathering', context, { resourceId });
      ...
      case 'use-item': {
        ...
        const context = { world: this.world, playerId: agentId };
        const result = await this.world.actionRegistry?.execute('use_item', context, { itemId, slot });
      ...
      case 'equip-item': {
        ...
        const context = { world: this.world, playerId: agentId };
        const result = await this.world.actionRegistry?.execute('equip_item', context, { itemId, slot });
      ...
      case 'pickup-item': {
        ...
        const context = { world: this.world, playerId: agentId };
        const result = await this.world.actionRegistry?.execute('pickup_item', context, { itemId });
      ...
      case 'drop-item': {
        ...
        const context = { world: this.world, playerId: agentId };
        const result = await this.world.actionRegistry?.execute('drop_item', context, { itemId, quantity });
      ...
      case 'open-bank': {
        ...
        const context = { world: this.world, playerId: agentId };
        const result = await this.world.actionRegistry?.execute('open_bank', context, { bankId });
      ...
      case 'deposit-item': {
        ...
        const context = { world: this.world, playerId: agentId };
        const result = await this.world.actionRegistry?.execute('deposit_item', context, { bankId, itemId, quantity });
      ...
      case 'withdraw-item': {
        ...
        const context = { world: this.world, playerId: agentId };
        const result = await this.world.actionRegistry?.execute('withdraw_item', context, { bankId, itemId, quantity });
      ...
      case 'buy-item': {
        ...
        const context = { world: this.world, playerId: agentId };
        const result = await this.world.actionRegistry?.execute('buy_item', context, { storeId, itemId, quantity });
      ...
      case 'sell-item': {
        ...
        const context = { world: this.world, playerId: agentId };
        const result = await this.world.actionRegistry?.execute('sell_item', context, { storeId, itemId, quantity });
