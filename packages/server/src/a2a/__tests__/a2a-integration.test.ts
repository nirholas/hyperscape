...
describe('Hyperscape A2A Integration - NO MOCKS', () => {
  ...
  describe('2. A2A Message Handling', () => {
    it('should handle get-status skill', async () => {
      ...
      const result = await (a2aServer as unknown as { executeSkill: (skillId: string, agentId: string, data: Record<string, unknown>) => Promise<{ success: boolean; message: string; data?: Record<string, unknown> }> }).executeSkill('get-status', agentId, data);
    ...
    it('should handle move-to skill', async () => {
      ...
      const result = await (a2aServer as unknown as { executeSkill: (skillId: string, agentId: string, data: Record<string, unknown>) => Promise<{ success: boolean; message: string; data?: Record<string, unknown> }> }).executeSkill('move-to', agentId, data);
    ...
    it('should handle get-skills skill', async () => {
      ...
      const result = await (a2aServer as unknown as { executeSkill: (skillId: string, agentId: string, data: Record<string, unknown>) => Promise<{ success: boolean; message: string; data?: Record<string, unknown> }> }).executeSkill('get-skills', agentId, data);
    ...
    it('should validate required parameters', async () => {
      ...
      const result = await (a2aServer as unknown as { executeSkill: (skillId: string, agentId: string, data: Record<string, unknown>) => Promise<{ success: boolean; message: string; data?: Record<string, unknown> }> }).executeSkill('attack', agentId, data);
    ...
    it('should validate JSON-RPC format', () => {
      const validRequest = {
        ...
        params: {
          message: {
            role: 'user' as const,
            parts: [{ kind: 'data', data: { skillId: 'get-status', agentId: 'test' } }],
            messageId: 'msg-123',
  ...
  describe('4. Skill Categories', () => {
    ...
    it('should have gathering skills', () => {
      const card = generateAgentCard(SERVER_URL);
      const gatheringSkills = card.skills.filter(s => s.tags.includes('gathering') || s.tags.includes('skills'));

      expect(gatheringSkills.length).toBeGreaterThanOrEqual(1);
      expect(gatheringSkills.some(s => s.id === 'gather-resource')).toBe(true);
    ...
    it('should have inventory skills', () => {
      const card = generateAgentCard(SERVER_URL);
      const inventorySkills = card.skills.filter(s => s.tags.includes('inventory'));
    ...
    it('should have economy skills', () => {
      const card = generateAgentCard(SERVER_URL);
      const economySkills = card.skills.filter(s => s.tags.includes('economy') || s.tags.includes('banking'));
