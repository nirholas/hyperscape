...
export class ERC8004RegistryClient {
  ...
  async initialize(): Promise<void> {
    ...
    if (!contractAddress || !abi) {
      throw new Error(
        "IdentityRegistry contract not found. Deploy contracts first.",
      );
    }
  ...
  async register(
    ...
    const resolution = await this.registryContract.resolveAgentByAddress(
      this.wallet.address,
    );

    if (resolution.agentId_ !== 0n) {
      console.log(
        `[ERC-8004] Already registered as Agent #${resolution.agentId_}`,
      );
      return {
        registered: true,
...
async function detectAndDeployContracts(): Promise<string | null> {
  ...
  if (chainId === 31337) {
    console.log(
      "[ERC-8004] Contracts not deployed - auto-deploying to Anvil...",
    );
    ...
    if (!existsSync(contractsDir)) {
      console.log(
        "[ERC-8004] Contracts directory not found - skipping deployment",
      );
      return null;
    }
    ...
    const deployProc = spawn({
      cmd: [
        "forge",
        "script",
        "script/DeployLiquiditySystem.s.sol:DeployLiquiditySystem",
        "--rpc-url",
        rpcUrl,
        "--broadcast",
      ],
    ...
    if (deployProc.exitCode === 0) {
      ...
      console.log(
        "[ERC-8004] Auto-deployment failed - continuing without registry",
      );
    }
  }
 ...
 */
export async function autoRegisterToRegistry(
  serverUrl: string,
  gameName: string,
): Promise<RegistrationResult> {
