...
export class ResponsiveWindowManager {
  ...
  getWindowDimensions(config?: WindowConfig) {
    ...
    return {
      ...
      maxHeight:
        screenSize === ScreenSize.MOBILE ? "calc(100vh - 80px)" : "80vh",
    };
  }
  ...
  getWindowPosition(windowId: string, config?: WindowConfig) {
    ...
    const desktopPositions: Record<string, { x: number; y: number }> = {
      ...
      equipment: {
        x: Math.min(
          ...
          viewportWidth - (windowSizes.equipment?.width || 500) - rightMargin,
        ),
        y: topMargin,
      },
      inventory: {
        x: Math.min(
          leftMargin + horizontalSpacing * 2,
          viewportWidth - (windowSizes.inventory?.width || 450) - rightMargin,
        ),
        y: topMargin,
      },
      settings: {
        x: Math.max(
          viewportWidth - (windowSizes.settings?.width || 380) - rightMargin,
          leftMargin,
        ),
        y: topMargin,
      },
      prefs: {
        x: Math.max(
          viewportWidth - (windowSizes.prefs?.width || 380) - rightMargin,
          leftMargin,
        ),
        y: topMargin,
      },
      ...
      account: {
        ...
        y: Math.min(
          ...
          viewportHeight - (windowSizes.account?.height || 550) - bottomMargin,
        ),
      },
      skills: {
        x: Math.min(
          leftMargin + horizontalSpacing,
          viewportWidth - (windowSizes.skills?.width || 450) - rightMargin,
        ),
        y: Math.min(
          topMargin + verticalSpacing,
          viewportHeight - (windowSizes.skills?.height || 550) - bottomMargin,
        ),
      },
    };
    ...
    if (defaultPos) {
      ...
      return {
        x: Math.max(
          leftMargin,
          Math.min(defaultPos.x, viewportWidth - 400 - rightMargin),
        ),
        y: Math.max(
          topMargin,
          Math.min(defaultPos.y, viewportHeight - 400 - bottomMargin),
        ),
      };
    }
