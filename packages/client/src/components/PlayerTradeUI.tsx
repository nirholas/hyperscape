...
interface TradeOffer {
  items: Array<{ itemId: string; quantity: number; slot: number; name: string }>;
  coins: number;
}
...
export function TradeRequestModal({
  ...
  useEffect(() => {
    const handler = (data: { tradeId: string; fromPlayerId: string; fromPlayerName: string }) => {
  ...
  const handleAccept = useCallback(() => {
    if (!request) return;
    console.log('[TradeRequestModal] Accepting trade request:', request.tradeId);
  ...
  const handleDecline = useCallback(() => {
    if (!request) return;
    console.log('[TradeRequestModal] Declining trade request:', request.tradeId);
...
function getItemIcon(itemId: string): string {
  ...
  if (itemId.includes('body') || itemId.includes('plate') || itemId.includes('chainbody')) return '🦺';
...
function DraggableInventoryItem({ 
  ...
  item: { slot: number; itemId: string; quantity: number; item: { name: string } }; 
  onClick: () => void;
}) {
    ...
    >
      {items.length === 0 ? (
        <div className="text-gray-500 text-center py-8 text-sm">
          {disabled ? 'Your offer is locked' : 'Drag items here or click items below'}
        </div>
      ) : (
                ...
                onContextMenu={(e) => {
                  ...
                  const evt = new CustomEvent('contextmenu', {
                    detail: {
                      ...
                      items: [
                        { id: 'remove', label: `Remove ${item.name}`, enabled: true },
                  ...
                  const onSelect = (selectEvt: Event) => {
                    const ce = selectEvt as CustomEvent<{ actionId: string; targetId: string }>;
...
export function TradeWindow({
  ...
  const [inventory, setInventory] = useState<Array<{ slot: number; itemId: string; quantity: number; item: { name: string } }>>([]);
  const [coins, setCoins] = useState<number>(0);
  const [offerCoins, setOfferCoins] = useState<string>('0');
  const [draggedItem, setDraggedItem] = useState<{ slot: number; itemId: string; quantity: number; item: { name: string } } | null>(null);

  // Helper to enrich trade offer items with names from item registry
  const enrichOfferWithNames = useCallback((offer: { items: Array<{ itemId: string; quantity: number; slot: number; name?: string }>; coins: number }): TradeOffer => {
    return {
      items: offer.items.map(item => {
        // If name already provided, use it
        if (item.name) return item as { itemId: string; quantity: number; slot: number; name: string };
  ...
  const handleAddItem = useCallback((item: { slot: number; itemId: string; quantity: number; item: { name: string } }) => {
    ...
    const alreadyInOffer = tradeState.yourOffer.items.some(i => i.slot === item.slot);
    if (alreadyInOffer) {
    ...
    console.log('My current offer before add:', tradeState.yourOffer.items.length, 'items');
    console.log('New offer will have:', newItems.length, 'items');
    console.log('I am:', tradeState.isInitiator ? '🔵 INITIATOR' : '🔴 RECIPIENT');
    console.log('Sending to server:', newItems.map(i => i.itemId).join(', '));
    console.log('═══════════════════════════════════════════════════════');
  ...
  const handleRemoveItem = useCallback((slot: number) => {
    if (!tradeState) return;
    ...
    const removedItem = tradeState.yourOffer.items.find(i => i.slot === slot);
    const newItems = tradeState.yourOffer.items.filter(i => i.slot !== slot);
  ...
  const handleUpdateCoins = useCallback(() => {
    ...
    if (coinAmount !== tradeState.yourOffer.coins) {
      world.emit(EventType.UI_TOAST, {
        message: coinAmount > 0 ? `Offering ${coinAmount} gold` : 'Removed gold from offer',
        type: 'info'
      });
    }
  ...
  const handleDragStart = useCallback((event: DragStartEvent) => {
    const itemData = event.active.data.current as { item: { slot: number; itemId: string; quantity: number; item: { name: string } } } | undefined;
    if (itemData?.item) {
  ...
  const handleDragEnd = useCallback((event: DragEndEvent) => {
    ...
    if (over && over.id === 'trade-offer-area' && draggedItem) {
      console.log('[TradeWindow] Dropped item into trade offer:', draggedItem.itemId);
  ...
  useEffect(() => {
    const handleTradeStarted = (data: {
      ...
      const myPlayerId = world.network?.socket?.player?.id 
      ...
      const otherPlayerName = isInitiator ? data.recipientName : data.initiatorName;
      
      console.log('═══════════════════════════════════════════════════════');
      console.log('[TradeWindow] 🎬 TRADE STARTED - WHO AM I?');
      console.log('═══════════════════════════════════════════════════════');
      console.log('Tried player IDs:');
      console.log('  world.network.socket.player.id:', world.network?.socket?.player?.id);
      ...
      console.log('  Initiator ID:', data.initiatorId, '(', data.initiatorName, ')');
      console.log('  Recipient ID:', data.recipientId, '(', data.recipientName, ')');
      console.log('');
      console.log('>>> I AM:', isInitiator ? '🔵 INITIATOR (I sent the request)' : '🔴 RECIPIENT (I accepted)');
  ...
  useEffect(() => {
    const handleTradeUpdated = (data: {
      tradeId: string;
      initiatorOffer: { items: Array<{ itemId: string; quantity: number; slot: number; name?: string }>; coins: number };
      recipientOffer: { items: Array<{ itemId: string; quantity: number; slot: number; name?: string }>; coins: number };
      ...
      setTradeState(prev => {
        ...
        console.log('─────────────────────────────────────────────────────');
        console.log('I am:', prev.isInitiator ? '🔵 INITIATOR' : '🔴 RECIPIENT');
        console.log('');
        console.log('Server sent:');
        console.log('  initiatorOffer (Player 1):', data.initiatorOffer.items.length, 'items,', data.initiatorOffer.coins, 'gold');
        console.log('  recipientOffer (Player 2):', data.recipientOffer.items.length, 'items,', data.recipientOffer.coins, 'gold');
        console.log('');
        ...
        const enrichedYourOffer = enrichOfferWithNames(prev.isInitiator ? data.initiatorOffer : data.recipientOffer);
        const enrichedTheirOffer = enrichOfferWithNames(prev.isInitiator ? data.recipientOffer : data.initiatorOffer);
        const newTheirConfirmed = prev.isInitiator ? data.recipientConfirmed : data.initiatorConfirmed;
        
        console.log('');
        console.log('Result:');
        console.log('  👤 MY offer (will show in GREEN on left):', enrichedYourOffer.items.length, 'items');
        console.log('  👥 THEIR offer (will show in BLUE on right):', enrichedTheirOffer.items.length, 'items');
        console.log('─────────────────────────────────────────────────────');
        
        // Check if other player updated their offer
        const theirItemsChanged = JSON.stringify(prev.theirOffer.items) !== JSON.stringify(enrichedTheirOffer.items);
        const theirCoinsChanged = prev.theirOffer.coins !== enrichedTheirOffer.coins;
        
        if (theirItemsChanged || theirCoinsChanged) {
          const changes = [];
          if (theirItemsChanged) {
            const itemNames = enrichedTheirOffer.items.map(i => `${i.name} x${i.quantity}`).join(', ');
            changes.push(itemNames || `${enrichedTheirOffer.items.length} items`);
          }
          if (theirCoinsChanged && enrichedTheirOffer.coins > 0) changes.push(`${enrichedTheirOffer.coins} gold`);
        ...
        return {
          ...
          yourConfirmed: prev.isInitiator ? data.initiatorConfirmed : data.recipientConfirmed,
  ...
  useEffect(() => {
    const handleInventoryUpdate = (raw: unknown) => {
      const data = raw as { 
        playerId: string; 
        items: Array<{ slot: number; itemId: string; quantity: number; item?: { name: string } }>; 
        coins: number;
      };
      
      const myPlayerId = world.network?.socket?.player?.id || world.entities?.player?.id;
    ...
    const requestInitial = () => {
      ...
      if (lp) {
        ...
        if (cached && Array.isArray(cached.items)) {
          const fullItems = cached.items.map((item: any) => ({
            slot: item.slot,
            itemId: item.itemId,
            quantity: item.quantity,
            item: { name: item.item?.name || item.itemId }
  ...
  const availableItems = inventory.filter(
    item => !tradeState.yourOffer.items.some(offered => offered.slot === item.slot)
  );

  // Calculate trade value estimates
  const yourOfferValue = tradeState.yourOffer.items.reduce((sum, item) => {
    const itemDef = getItem(item.itemId);
    return sum + ((itemDef?.value || 0) * item.quantity);
  }, 0) + tradeState.yourOffer.coins;

  const theirOfferValue = tradeState.theirOffer.items.reduce((sum, item) => {
    const itemDef = getItem(item.itemId);
    return sum + ((itemDef?.value || 0) * item.quantity);
      ...
      >
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <h2 className="text-2xl font-bold text-yellow-400">
            ...
            <div className="text-xs text-gray-400 mt-1">
              Trade ID: {tradeState.tradeId.slice(0, 8)}... | 
              You are: {tradeState.isInitiator ? 'Initiator (sent request)' : 'Recipient (accepted request)'}
            </div>
          </div>
          <button
            onClick={(e) => {
        ...
        <div className="mb-4 bg-gray-800 p-3 rounded">
          <div className="flex justify-between mb-2">
            <div className={`flex items-center gap-2 font-semibold ${tradeState.yourConfirmed ? 'text-green-400' : 'text-gray-400'}`}>
              {tradeState.yourConfirmed ? '✅' : '⭕'} You
              {tradeState.yourConfirmed && <span className="text-xs">(Confirmed)</span>}
            </div>
            <div className={`flex items-center gap-2 font-semibold ${tradeState.theirConfirmed ? 'text-green-400' : 'text-gray-400'}`}>
              {tradeState.theirConfirmed ? '✅' : '⭕'} {tradeState.otherPlayerName}
              {tradeState.theirConfirmed && <span className="text-xs">(Confirmed)</span>}
            </div>
          </div>
        ...
        {(yourOfferValue > 0 || theirOfferValue > 0) && (
          <div className="mb-6 bg-gray-800 p-3 rounded text-center">
            ...
            {Math.abs(yourOfferValue - theirOfferValue) > yourOfferValue * 0.5 && (
 ...
 */
export function TradeSuccessNotification(): JSX.Element {
  return (
    <div 
