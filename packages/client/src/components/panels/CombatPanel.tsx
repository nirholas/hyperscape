...
import type { ClientWorld, PlayerStats, PlayerEquipmentItems, PlayerHealth } from '../../types'
...
export function CombatPanel({ world, stats, equipment }: CombatPanelProps) {
  ...
  useEffect(() => {
    ...
    const onTargetChanged = (data: unknown) => {
      const d = data as { targetId: string | null; targetName?: string; targetHealth?: PlayerHealth }
      if (d.targetId) {
    ...
    world.on(EventType.UI_COMBAT_TARGET_HEALTH, onTargetHealthUpdate, undefined)
    ...
    return () => {
      ...
      world.off(EventType.UI_COMBAT_TARGET_CHANGED, onTargetChanged, undefined, undefined)
      world.off(EventType.UI_COMBAT_TARGET_HEALTH, onTargetHealthUpdate, undefined, undefined)
    }
  }, [world, targetName])
  ...
  const targetHealthPercent = targetHealth ? Math.round((targetHealth.current / targetHealth.max) * 100) : 0

  return (
    <div className="flex flex-col gap-1.5">
      {/* Player Health Bar */}
      <div className="flex flex-col gap-0.5">
        <div className="flex items-center justify-between text-[10px]" style={{ color: '#f2d08a' }}>
        ...
        <div className="w-full bg-black/50 rounded-full h-2.5 border border-white/20 overflow-hidden">
          <div
            ...
            style={{
              width: `${healthPercent}%`,
              background: healthPercent > 50 ? 'linear-gradient(90deg, #22c55e, #16a34a)' : healthPercent > 25 ? 'linear-gradient(90deg, #f59e0b, #d97706)' : 'linear-gradient(90deg, #ef4444, #dc2626)'
            }}
          />
        </div>
      </div>
      ...
      {targetName && targetHealth && (
        <div className="flex flex-col gap-0.5 bg-red-900/20 border border-red-500/30 rounded-md p-1">
          <div className="flex items-center justify-between text-[9px]" style={{ color: '#fca5a5' }}>
      ...
      <div className="grid grid-cols-3 gap-1">
        ...
        >
          ...
          <div className="font-semibold text-xs" style={{ color: '#f2d08a' }}>{attackLevel}</div>
        </div>
        <div
          className="bg-black/35 border rounded-md p-1 flex flex-col items-center justify-center"
          style={{ borderColor: 'rgba(242, 208, 138, 0.3)' }}
        >
          ...
          <div className="font-semibold text-xs" style={{ color: '#f2d08a' }}>{strengthLevel}</div>
        </div>
        <div
          className="bg-black/35 border rounded-md p-1 flex flex-col items-center justify-center"
          style={{ borderColor: 'rgba(242, 208, 138, 0.3)' }}
        >
          ...
          <div className="font-semibold text-xs" style={{ color: '#f2d08a' }}>{defenseLevel}</div>
        </div>
      </div>
      ...
      {/* Attack Style */}
      <div className="font-semibold mt-0.5 text-[10px]" style={{ color: '#f2d08a' }}>Attack style</div>
      ...
      {cooldown > 0 && (
        <div className="text-[9px]" style={{ color: 'rgba(242, 208, 138, 0.6)' }}>
