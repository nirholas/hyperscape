...
function SkillBox({ skill, onHover, onLeave }: {
  skill: Skill
  ...
  return (
    <button
      ...
      style={{
        background: 'linear-gradient(to bottom, rgba(45, 35, 25, 0.95) 0%, rgba(30, 25, 20, 0.95) 100%)',
        border: '1px solid #5c4a3a',
        borderRadius: '3px',
        boxShadow: 'inset 1px 1px 0 rgba(80, 65, 50, 0.8), inset -1px -1px 0 rgba(20, 15, 10, 0.9), 0 1px 3px rgba(0, 0, 0, 0.8)',
        cursor: 'default',
        width: '100%',
        minHeight: '40px',
      }}
    >
      ...
      <div className="flex-1 flex items-center justify-center w-full">
        <div className="text-base" style={{ filter: 'drop-shadow(1px 1px 1px rgba(0, 0, 0, 0.8))' }}>
          {skill.icon}
        </div>
      </div>
...
function PrayerCard({ prayer, onToggle }: { prayer: Prayer; onToggle: (id: string) => void }) {
  ...
  return (
    <button
      ...
      style={{
        ...
        borderColor: prayer.active ? 'rgba(34, 197, 94, 0.6)' : 'rgba(242, 208, 138, 0.35)',
      ...
      <div className="flex items-center justify-between gap-0.5">
        ...
        <div
          ...
          style={{
            ...
            borderWidth: '1px',
            borderStyle: 'solid',
            borderColor: prayer.active ? 'rgba(34, 197, 94, 0.4)' : 'rgba(242, 208, 138, 0.2)',
          }}
        >
          {prayer.icon}
        </div>
        ...
        <div className="flex items-center gap-0.5 flex-shrink-0">
          <div className="text-[10px] font-semibold" style={{ color: 'rgba(242, 208, 138, 0.8)' }}>
            {prayer.level}
          </div>
          {/* Active Indicator */}
          {prayer.active && (
      ...
      {showTooltip && (
        <div
          ...
          style={{
            ...
            background: 'linear-gradient(135deg, rgba(20, 20, 30, 0.98) 0%, rgba(25, 20, 35, 0.96) 100%)',
        ...
        >
          <div className="font-semibold mb-0.5 text-[10px]" style={{ color: '#f2d08a' }}>
            {prayer.name}
          </div>
          <div
          ...
          <div className="text-[9px]" style={{ color: 'rgba(242, 208, 138, 0.7)' }}>
...
export function SkillsPanel({ world: _world, stats }: SkillsPanelProps) {
  ...
  const prayers: Prayer[] = [
    // Offensive Prayers
    { id: 'clarity', name: 'Clarity', icon: '🧠', level: 1, description: 'Increases accuracy by 5%', drainRate: 3, active: activePrayers.has('clarity'), category: 'offensive' },
    { id: 'strength', name: 'Strength', icon: '💪', level: 4, description: 'Increases max hit by 5%', drainRate: 3, active: activePrayers.has('strength'), category: 'offensive' },
    { id: 'sharpEye', name: 'Sharp Eye', icon: '🎯', level: 8, description: 'Increases ranged accuracy by 5%', drainRate: 3, active: activePrayers.has('sharpEye'), category: 'offensive' },
    { id: 'burst', name: 'Burst of Strength', icon: '⚡', level: 16, description: 'Increases max hit by 10%', drainRate: 6, active: activePrayers.has('burst'), category: 'offensive' },

    // Defensive Prayers
    { id: 'thickSkin', name: 'Thick Skin', icon: '🛡️', level: 1, description: 'Increases defense by 5%', drainRate: 3, active: activePrayers.has('thickSkin'), category: 'defensive' },
    { id: 'rockSkin', name: 'Rock Skin', icon: '🪨', level: 10, description: 'Increases defense by 10%', drainRate: 6, active: activePrayers.has('rockSkin'), category: 'defensive' },
    { id: 'steelSkin', name: 'Steel Skin', icon: '⚙️', level: 28, description: 'Increases defense by 15%', drainRate: 12, active: activePrayers.has('steelSkin'), category: 'defensive' },
    { id: 'protect', name: 'Protect from Melee', icon: '🔰', level: 43, description: 'Blocks 40% melee damage', drainRate: 12, active: activePrayers.has('protect'), category: 'defensive' },

    // Utility Prayers
    { id: 'rapidHeal', name: 'Rapid Heal', icon: '❤️‍🩹', level: 22, description: 'Doubles health regeneration', drainRate: 6, active: activePrayers.has('rapidHeal'), category: 'utility' },
    { id: 'preserveGather', name: 'Preserve', icon: '🌿', level: 35, description: 'Reduces gathering drain by 50%', drainRate: 3, active: activePrayers.has('preserveGather'), category: 'utility' },
  ]
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden gap-1">
        ...
        <button
          ...
          style={{
            background: activeTab === 'skills'
              ...
              : 'linear-gradient(135deg, rgba(20, 20, 30, 0.95) 0%, rgba(25, 20, 35, 0.92) 100%)',
            borderColor: activeTab === 'skills' ? 'rgba(242, 208, 138, 0.6)' : 'rgba(242, 208, 138, 0.35)',
            boxShadow: activeTab === 'skills'
              ? '0 0 8px rgba(242, 208, 138, 0.2), 0 1px 3px rgba(0, 0, 0, 0.5)'
              : '0 1px 3px rgba(0, 0, 0, 0.5)',
            color: activeTab === 'skills' ? '#f2d08a' : 'rgba(242, 208, 138, 0.7)',
        ...
        <button
          ...
          style={{
            background: activeTab === 'prayer'
              ? 'linear-gradient(135deg, rgba(242, 208, 138, 0.2) 0%, rgba(242, 208, 138, 0.1) 100%)'
              : 'linear-gradient(135deg, rgba(20, 20, 30, 0.95) 0%, rgba(25, 20, 35, 0.92) 100%)',
            borderColor: activeTab === 'prayer' ? 'rgba(242, 208, 138, 0.6)' : 'rgba(242, 208, 138, 0.35)',
            boxShadow: activeTab === 'prayer'
              ? '0 0 8px rgba(242, 208, 138, 0.2), 0 1px 3px rgba(0, 0, 0, 0.5)'
              : '0 1px 3px rgba(0, 0, 0, 0.5)',
            color: activeTab === 'prayer' ? '#f2d08a' : 'rgba(242, 208, 138, 0.7)',
      ...
      {/* Content */}
      <div className="flex-1 overflow-y-auto noscrollbar" onMouseMove={handleMouseMove}>
        {activeTab === 'skills' ? (
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <div className="flex items-center justify-between">
                <span className="font-medium text-[10px]" style={{ color: '#c9b386' }}>
                  Total level:
                </span>
                <span
              ...
              <div className="flex items-center justify-between">
                ...
                >
                  {Math.floor((s.attack?.level || 1) * 0.25 + (s.strength?.level || 1) * 0.25 + (s.defense?.level || 1) * 0.25 + (s.constitution?.level || 10) * 0.25)}
                </span>
              </div>
          ...
          <div className="flex flex-col gap-1.5">
            {/* Offensive Prayers */}
            <div>
              <div className="font-semibold mb-1 text-[10px]" style={{ color: '#f2d08a' }}>
                ⚔️ Offensive
              </div>
              <div className="grid grid-cols-2 gap-1">
                {offensivePrayers.map(prayer => (
                  <PrayerCard key={prayer.id} prayer={prayer} onToggle={togglePrayer} />
                ))}
              </div>
            </div>
      ...
      {activeTab === 'skills' && hoveredSkill && (() => {
        ...
        const progress = Math.min(100, Math.max(0, (xpIntoLevel / xpForThisLevel) * 100))
          ...
          >
            <div className="flex items-center justify-between mb-1">
              <div className="flex items-center gap-1">
                ...
                <span className="font-semibold text-[10px]" style={{ color: '#f2d08a' }}>
                  {hoveredSkill.label}
                </span>
              </div>
              <span
                className="font-bold text-[10px]"
                style={{ color: hoveredSkill.level >= 99 ? '#fbbf24' : '#f2d08a' }}
              >
                {hoveredSkill.level}
              </span>
            </div>
            ...
            {hoveredSkill.key !== 'total' ? (
              <>
                <div className="mb-0.5 text-[9px]" style={{ color: 'rgba(242, 208, 138, 0.8)' }}>
                  XP: {Math.floor(hoveredSkill.xp).toLocaleString()}
                </div>
                <div className="mb-1 text-[9px]" style={{ color: 'rgba(242, 208, 138, 0.8)' }}>
                  Next level: {xpRemaining.toLocaleString()} XP
                </div>

                <div className="rounded overflow-hidden h-1.5" style={{ background: 'rgba(0, 0, 0, 0.5)' }}>
                  <div
                    className="h-full transition-all duration-300"
                    style={{
                      width: `${progress}%`,
                      background: hoveredSkill.level >= 99
                      ...
                      boxShadow: hoveredSkill.level >= 99
              ...
              <div className="text-[9px]" style={{ color: 'rgba(242, 208, 138, 0.8)' }}>
