...
function DroppableEquipmentSlot({ slot, onSlotClick }: DroppableEquipmentSlotProps) {
...
export function EquipmentPanel({ equipment, stats, onItemDrop: _onItemDrop }: EquipmentPanelProps) {
  const [selectedSlot, setSelectedSlot] = useState<EquipmentSlot | null>(null)

  // Equipment slots with icons for paperdoll layout
  const slots: EquipmentSlot[] = [
    { key: EquipmentSlotName.HELMET, label: 'Head', icon: '⛑️', item: equipment?.helmet || null },
    { key: EquipmentSlotName.BODY, label: 'Body', icon: '🎽', item: equipment?.body || null },
    { key: EquipmentSlotName.LEGS, label: 'Legs', icon: '👖', item: equipment?.legs || null },
    { key: EquipmentSlotName.WEAPON, label: 'Weapon', icon: '⚔️', item: equipment?.weapon || null },
    { key: EquipmentSlotName.SHIELD, label: 'Shield', icon: '🛡️', item: equipment?.shield || null },
    { key: EquipmentSlotName.ARROWS, label: 'Ammo', icon: '🏹', item: equipment?.arrows || null },
  ]

  // Calculate total bonuses from equipped items
  const totalStats = slots.reduce((acc, slot) => {
    if (slot.item) {
        ...
        <div className="flex-1 flex overflow-hidden gap-1">
          ...
          <div className="flex-1 flex flex-col" style={{ minWidth: '60%' }}>
            ...
            >
              ...
              <div className="grid grid-cols-3 gap-1 h-full" style={{ gridTemplateRows: 'repeat(4, 1fr)' }}>
          ...
          <div className="flex flex-col gap-1" style={{ width: '40%' }}>
            ...
            >
              ...
              <div className="flex items-center justify-center mb-1 gap-1.5">
                <div style={{ fontSize: 'clamp(0.625rem, 1.1vw, 0.75rem)', color: 'rgba(242, 208, 138, 0.8)' }}>
                  Lvl <span className="font-bold" style={{ color: '#f2d08a' }}>{playerLevel}</span>
                </div>
                <div style={{ fontSize: 'clamp(0.625rem, 1.1vw, 0.75rem)', color: 'rgba(242, 208, 138, 0.8)' }}>
                  CB <span className="font-bold" style={{ color: '#f2d08a' }}>{combatLevel}</span>
                </div>
              </div>

              {/* Health */}
              <div className="flex items-center mb-0.5 gap-1">
                <span style={{ fontSize: 'clamp(0.563rem, 1vw, 0.625rem)' }}>❤️</span>
                <span style={{ fontSize: 'clamp(0.563rem, 1vw, 0.625rem)', color: 'rgba(242, 208, 138, 0.9)', fontWeight: 'bold' }}>
              ...
              >
                <div
                  ...
                  style={{
                    background: 'linear-gradient(90deg, rgba(220, 38, 38, 0.8) 0%, rgba(239, 68, 68, 0.9) 100%)',
            ...
            >
              <div style={{ fontSize: 'clamp(0.563rem, 1vw, 0.625rem)', color: 'rgba(242, 208, 138, 0.9)', fontWeight: 'bold', marginBottom: 'clamp(0.25rem, 0.5vw, 0.375rem)' }}>
                Combat Stats
              </div>
              <div className="space-y-0.5">
                {/* Attack */}
                <div className="flex justify-between items-center">
                  <div className="flex items-center" style={{ gap: 'clamp(0.125rem, 0.3vw, 0.2rem)' }}>
                    <span style={{ fontSize: 'clamp(0.563rem, 1vw, 0.625rem)' }}>⚔️</span>
                    <span style={{ fontSize: 'clamp(0.563rem, 1vw, 0.625rem)', color: 'rgba(242, 208, 138, 0.8)' }}>Attack</span>
                  </div>
                  <div style={{ fontSize: 'clamp(0.563rem, 1vw, 0.625rem)', color: '#f2d08a', fontWeight: 'bold' }}>
                    {attackSkill}
                    {totalStats.attack > 0 && (
                      <span style={{ color: '#22c55e', fontSize: 'clamp(0.5rem, 0.9vw, 0.563rem)' }}> +{totalStats.attack}</span>
                    )}
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center">
                  <div className="flex items-center" style={{ gap: 'clamp(0.125rem, 0.3vw, 0.2rem)' }}>
                    <span style={{ fontSize: 'clamp(0.563rem, 1vw, 0.625rem)' }}>💪</span>
                    <span style={{ fontSize: 'clamp(0.563rem, 1vw, 0.625rem)', color: 'rgba(242, 208, 138, 0.8)' }}>Strength</span>
                  </div>
                  <div style={{ fontSize: 'clamp(0.563rem, 1vw, 0.625rem)', color: '#f2d08a', fontWeight: 'bold' }}>
                    {strengthSkill}
                    {totalStats.strength > 0 && (
                      <span style={{ color: '#22c55e', fontSize: 'clamp(0.5rem, 0.9vw, 0.563rem)' }}> +{totalStats.strength}</span>
                    )}
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center">
                  <div className="flex items-center" style={{ gap: 'clamp(0.125rem, 0.3vw, 0.2rem)' }}>
                    <span style={{ fontSize: 'clamp(0.563rem, 1vw, 0.625rem)' }}>🛡️</span>
                    <span style={{ fontSize: 'clamp(0.563rem, 1vw, 0.625rem)', color: 'rgba(242, 208, 138, 0.8)' }}>Defense</span>
                  </div>
                  <div style={{ fontSize: 'clamp(0.563rem, 1vw, 0.625rem)', color: '#f2d08a', fontWeight: 'bold' }}>
                    {defenseSkill}
                    {totalStats.defense > 0 && (
                      <span style={{ color: '#22c55e', fontSize: 'clamp(0.5rem, 0.9vw, 0.563rem)' }}> +{totalStats.defense}</span>
                    )}
                  </div>
                </div>
              </div>
              ...
              >
                ...
                <div className="space-y-0.5">
                  <div className="flex justify-between">
                    <span style={{ fontSize: 'clamp(0.563rem, 1vw, 0.625rem)', color: 'rgba(242, 208, 138, 0.8)' }}>⚔️ Attack Bonus</span>
                    <span style={{ fontSize: 'clamp(0.563rem, 1vw, 0.625rem)', color: '#22c55e', fontWeight: 'bold' }}>
                      +{totalStats.attack}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span style={{ fontSize: 'clamp(0.563rem, 1vw, 0.625rem)', color: 'rgba(242, 208, 138, 0.8)' }}>🛡️ Defense Bonus</span>
                    <span style={{ fontSize: 'clamp(0.563rem, 1vw, 0.625rem)', color: '#22c55e', fontWeight: 'bold' }}>
                      +{totalStats.defense}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span style={{ fontSize: 'clamp(0.563rem, 1vw, 0.625rem)', color: 'rgba(242, 208, 138, 0.8)' }}>💪 Strength Bonus</span>
                    <span style={{ fontSize: 'clamp(0.563rem, 1vw, 0.625rem)', color: '#22c55e', fontWeight: 'bold' }}>
                      +{totalStats.strength}
                    </span>
                  </div>
              ...
              >
                <div className="flex justify-between">
                  <span style={{ fontSize: 'clamp(0.563rem, 1vw, 0.625rem)', color: 'rgba(242, 208, 138, 0.8)' }}>⚖️ Total Weight</span>
                  <span style={{ fontSize: 'clamp(0.563rem, 1vw, 0.625rem)', color: '#f2d08a', fontWeight: 'bold' }}>
                    {slots.reduce((sum, slot) => sum + (slot.item?.weight || 0) * (slot.item?.quantity || 0), 0).toFixed(1)} kg
                  </span>
                </div>
              </div>
            </div>
            ...
            >
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center" style={{ gap: 'clamp(0.375rem, 0.8vw, 0.5rem)' }}>
                  <span style={{ fontSize: 'clamp(1.25rem, 2.5vw, 1.5rem)' }}>{selectedSlot.icon}</span>
                  <div>
              ...
              {/* Stats */}
              {(item.stats.attack > 0 || item.stats.defense > 0 || item.stats.strength > 0) && (
                <div className="mb-3">
                  <div className="mb-1" style={{ fontSize: 'clamp(0.688rem, 1.2vw, 0.75rem)', color: 'rgba(242, 208, 138, 0.9)', fontWeight: 'bold' }}>
                    Combat Stats
                  </div>
                  <div className="space-y-1">
                    {item.stats.attack > 0 && (
                      <div className="flex justify-between" style={{ fontSize: 'clamp(0.625rem, 1.1vw, 0.75rem)', color: 'rgba(242, 208, 138, 0.8)' }}>
                        <span>⚔️ Attack</span>
                        <span style={{ color: '#22c55e', fontWeight: 'bold' }}>+{item.stats.attack}</span>
                      </div>
                    )}
                    {item.stats.defense > 0 && (
                      <div className="flex justify-between" style={{ fontSize: 'clamp(0.625rem, 1.1vw, 0.75rem)', color: 'rgba(242, 208, 138, 0.8)' }}>
                        <span>🛡️ Defense</span>
                        <span style={{ color: '#22c55e', fontWeight: 'bold' }}>+{item.stats.defense}</span>
                      </div>
                    )}
                    {item.stats.strength > 0 && (
                      <div className="flex justify-between" style={{ fontSize: 'clamp(0.625rem, 1.1vw, 0.75rem)', color: 'rgba(242, 208, 138, 0.8)' }}>
                        <span>💪 Strength</span>
                        <span style={{ color: '#22c55e', fontWeight: 'bold' }}>+{item.stats.strength}</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {/* Requirements */}
              {(item.requirements.level > 1 || Object.keys(item.requirements.skills || {}).length > 0) && (
                <div className="mb-3">
                  <div className="mb-1" style={{ fontSize: 'clamp(0.688rem, 1.2vw, 0.75rem)', color: 'rgba(242, 208, 138, 0.9)', fontWeight: 'bold' }}>
                    Requirements
                  </div>
                  <div className="space-y-1">
                    ...
                    {item.requirements.skills && Object.entries(item.requirements.skills).map(([skill, level]) => (
                      <div key={skill} style={{ fontSize: 'clamp(0.625rem, 1.1vw, 0.75rem)', color: 'rgba(242, 208, 138, 0.8)' }}>
              ...
              <div className="space-y-1">
                <div className="flex justify-between" style={{ fontSize: 'clamp(0.625rem, 1.1vw, 0.75rem)', color: 'rgba(242, 208, 138, 0.8)' }}>
                  <span>Value</span>
                  <span style={{ color: '#f2d08a', fontWeight: 'bold' }}>{item.value} coins</span>
                </div>
                <div className="flex justify-between" style={{ fontSize: 'clamp(0.625rem, 1.1vw, 0.75rem)', color: 'rgba(242, 208, 138, 0.8)' }}>
                  <span>Weight</span>
                  <span style={{ color: '#f2d08a', fontWeight: 'bold' }}>{item.weight} kg</span>
                </div>
                {item.quantity > 1 && (
                  <div className="flex justify-between" style={{ fontSize: 'clamp(0.625rem, 1.1vw, 0.75rem)', color: 'rgba(242, 208, 138, 0.8)' }}>
                    <span>Quantity</span>
                    <span style={{ color: '#f2d08a', fontWeight: 'bold' }}>{item.quantity}</span>
                  </div>
                )}
              </div>
              ...
              <button
                ...
                style={{
                  background: 'linear-gradient(135deg, rgba(139, 69, 19, 0.3) 0%, rgba(139, 69, 19, 0.2) 100%)',
