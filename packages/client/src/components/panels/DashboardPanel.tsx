...
import type { ClientWorld, PlayerStats, PlayerEquipmentItems, InventorySlotItem } from '../../types'
...
type TabType = 'account' | 'combat' | 'skills' | 'inventory' | 'equipment' | 'settings'
...
const tabs: Tab[] = [
  { id: 'account', icon: '👤', label: 'Profile', shortLabel: 'Prof', windowTitle: 'Account' },
  { id: 'combat', icon: '⚔️', label: 'Combat', shortLabel: 'Cmbt', windowTitle: 'Combat' },
  { id: 'skills', icon: '📊', label: 'Skills', shortLabel: 'Skll', windowTitle: 'Skills' },
  { id: 'inventory', icon: '🎒', label: 'Items', shortLabel: 'Invt', windowTitle: 'Inventory' },
  { id: 'equipment', icon: '🛡️', label: 'Gear', shortLabel: 'Gear', windowTitle: 'Equipment' },
  { id: 'settings', icon: '⚙️', label: 'Config', shortLabel: 'Cfg', windowTitle: 'Settings' },
]

export function DashboardPanel({ world, stats, equipment, inventory, coins, onOpenWindow }: DashboardPanelProps) {
        ...
        >
          ...
          <div
            ...
            style={{
              ...
              background: 'linear-gradient(180deg, rgba(30, 20, 10, 0.6) 0%, rgba(20, 15, 10, 0.4) 100%)',
          ...
          >
            ...
            >
              <span className="flex items-center gap-0.5">
                <span>↗</span>
                <span className="text-[8px] uppercase tracking-wider">Pop Out</span>
              </span>
            </button>
          </div>

          {/* Panel Content */}
          <div className="flex-1 overflow-y-auto noscrollbar p-2" style={{ fontSize: '1rem' }}>
            {activeTab === 'account' && <AccountPanel world={world} />}
            {activeTab === 'combat' && <CombatPanel world={world} stats={stats} equipment={equipment} />}
            {activeTab === 'skills' && <SkillsPanel world={world} stats={stats} />}
            {activeTab === 'inventory' && <InventoryPanel items={inventory} coins={coins} world={world} />}
            {activeTab === 'equipment' && <EquipmentPanel equipment={equipment} stats={stats} />}
            {activeTab === 'settings' && <SettingsPanel world={world} />}
          </div>
        </div>
      ) : (
        <div className="flex-1 overflow-y-auto noscrollbar" style={{ fontSize: '0.875rem' }}>
          {activeTab === 'account' && <AccountPanel world={world} />}
          {activeTab === 'combat' && <CombatPanel world={world} stats={stats} equipment={equipment} />}
          {activeTab === 'skills' && <SkillsPanel world={world} stats={stats} />}
          {activeTab === 'inventory' && <InventoryPanel items={inventory} coins={coins} world={world} />}
        ...
        {tabs.map((tab) => (
          <button
            ...
            style={{
              ...
              borderColor: activeTab === tab.id ? 'rgba(242, 208, 138, 0.7)' : 'rgba(242, 208, 138, 0.25)',
          ...
          >
            ...
            {activeTab === tab.id && (
              <div
                ...
                style={{
                  background: 'linear-gradient(180deg, #fbbf24 0%, #f59e0b 50%, #f2d08a 100%)',
                  boxShadow: '0 0 12px rgba(251, 191, 36, 0.8), inset 0 0 4px rgba(255, 255, 255, 0.3)',
                }}
              />
            )}
            ...
            {!isMobile && (
              <div
                ...
                style={{
                  color: activeTab === tab.id ? '#fbbf24' : 'rgba(242, 208, 138, 0.65)',
