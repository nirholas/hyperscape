...
export function DraggableWindow({
  ...
  const validatePosition = useCallback(
    (
      ...
      validatedPos.x = Math.max(
        margins.left,
        Math.min(validatedPos.x, viewport.width - windowWidth - margins.right),
      );
      validatedPos.y = Math.max(
        margins.top,
        Math.min(
          validatedPos.y,
          viewport.height - windowHeight - margins.bottom,
        ),
      );
  ...
  useEffect(() => {
    if (windowRef.current && !hasValidatedRef.current) {
      ...
      if (
        Math.abs(validatedPos.x - currentPos.x) > 5 ||
        Math.abs(validatedPos.y - currentPos.y) > 5
      ) {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (windowElement) {
        ...
        newPosition.x = Math.max(
          0,
          Math.min(newPosition.x, viewport.width - rect.width),
        );
        newPosition.y = Math.max(
          0,
          Math.min(newPosition.y, viewport.height - rect.height),
        );
      }
