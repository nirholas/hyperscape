...
export type RigType = 'mixamo-human' | 'mixamo-quadruped' | 'mixamo-bird' | 'custom'
    ...
    persist(
      subscribeWithSelector(
        immer((set, get) => ({
          ...
          setSourceModel: (url, assetId) => set(state => {
          ...
          setTargetRig: (type, url, assetId) => set(state => {
          ...
          setSourceSkeleton: (skeleton) => set(state => {
            state.sourceSkeleton = skeleton as any
            console.log('[Store] Source skeleton updated:', skeleton ? `${skeleton.bones.length} bones` : 'null')
          }),

          setTargetSkeleton: (skeleton) => set(state => {
            state.targetSkeleton = skeleton as any
            console.log('[Store] Target skeleton updated:', skeleton ? `${skeleton.bones.length} bones` : 'null')
          }),

          setRetargeted: (retargeted, modelUrl) => set(state => {
          ...
          setBoneMapping: (mapping) => set(state => {
            state.boneMapping = mapping
            console.log('[Store] Bone mapping set:', Object.keys(mapping).length, 'mappings')
          }),

          updateBoneMapping: (sourceBone, targetBone) => set(state => {
            state.boneMapping[sourceBone] = targetBone
            console.log('[Store] Added mapping:', sourceBone, '→', targetBone)
          }),

          removeBoneMapping: (sourceBone) => set(state => {
          ...
          setMirrorEnabled: (enabled) => set(state => {
          ...
          canEditBones: () => {
            const state = get()
            return state.isRetargeted && !!(state.sourceSkeleton && state.targetSkeleton)
          },
...
export const useCanRetarget = () => useRetargetingStore(state => state.canRetarget())
export const useCanEditBones = () => useRetargetingStore(state => state.canEditBones())
export const useCanExport = () => useRetargetingStore(state => state.canExport())
export const useMappedBonesCount = () => useRetargetingStore(state => state.getMappedBonesCount())
