...
export class VRMConverter {
  ...
  private normalizeScale(): void {
    ...
    if (!hipsBone || !headBone) {
      console.log('   ⚠️  Could not find hips/head bones for scale normalization')
    ...
    if (Math.abs(scaleFactor - 1.0) > 0.1) {
      ...
      if (this.skinnedMesh && this.skinnedMesh.geometry) {
        ...
        console.log('   ✅ Scaled geometry vertices by', scaleFactor.toFixed(3))
      }
      ...
      console.log(`   ✅ Scaled ${this.bones.length} bone positions by ${scaleFactor.toFixed(3)}`)
      ...
      if (this.skinnedMesh.skeleton) {
        this.skinnedMesh.skeleton.calculateInverses()
        console.log('   ✅ Recalculated inverse bind matrices after height scaling')
      }
  ...
  private verifyMeshSkeletonAlignment(): void {
    ...
    if (heightRatio < 0.5 || heightRatio > 2.0) {
      ...
      console.log(`   ✅ Mesh and skeleton alignment OK (height ratio: ${heightRatio.toFixed(2)}x)`)
    }
  }
  ...
  private ensureHipsTranslation(): void {
    ...
    const hipsBone = this.findBoneByName('Hips')
    if (!hipsBone) {
      console.warn('   ⚠️  Cannot ensure Hips translation - Hips bone not found')
      return
    }
    ...
    if (parent && parent.type !== 'Bone') {
      ...
      console.log(`   ✅ Baked world position into Hips local: [${hipsBone.position.x.toFixed(3)}, ${hipsBone.position.y.toFixed(3)}, ${hipsBone.position.z.toFixed(3)}]`)
      ...
      console.log('   ✅ Hips parent is a Bone - local position already correct')
    }
  }
  ...
  private async exportVRM(options: VRMConversionOptions): Promise<ArrayBuffer> {
    ...
    console.log('   ✅ All bones configured for TRS export (matrixAutoUpdate enabled)')

    // If model had non-T-pose bind pose, it was normalized in validateTPose()
    console.log('✅ Exporting VRM with T-pose bind pose for Hyperscape compatibility')
    ...
    if (gltfJson.nodes) {
      gltfJson.nodes.forEach((node: any, index: number) => {
        if (node.name === 'Hips' || node.name === 'Spine' || node.name === 'LeftArm' || node.name === 'RightArm') {
          ...
          if (node.matrix) {
            console.log(`     ⚠️  MATRIX STILL EXISTS: [${node.matrix.map((v: number) => v.toFixed(3)).join(', ')}]`)
          }
        }
      })
    }
    ...
    for (const [meshyBoneName, vrmBoneName] of this.boneMappings.entries()) {
      ...
      if (nodeIndex !== undefined) {
        ...
        console.warn(`   ⚠️  Could not find node index for bone: ${meshyBoneName}`)
      }
    }
