...
export const MESHY_VARIATIONS: Record<string, string[]> = {
  ...
  'LeftArm': ['LeftArm', 'LeftUpperArm', 'upper_arm.L', 'upperarm_L', 'L_UpperArm'],
  'LeftForeArm': ['LeftForeArm', 'LeftLowerArm', 'forearm.L', 'forearm_L', 'L_ForeArm'],
  'LeftHand': ['LeftHand', 'hand.L', 'hand_L', 'L_Hand'],

  'RightShoulder': ['RightShoulder', 'shoulder.R', 'shoulder_R', 'R_Shoulder'],
  'RightArm': ['RightArm', 'RightUpperArm', 'upper_arm.R', 'upperarm_R', 'R_UpperArm'],
  'RightForeArm': ['RightForeArm', 'RightLowerArm', 'forearm.R', 'forearm_R', 'R_ForeArm'],
  'RightHand': ['RightHand', 'hand.R', 'hand_R', 'R_Hand'],

  'LeftUpLeg': ['LeftUpLeg', 'LeftThigh', 'thigh.L', 'thigh_L', 'L_UpLeg', 'L_Thigh'],
  ...
  'RightUpLeg': ['RightUpLeg', 'RightThigh', 'thigh.R', 'thigh_R', 'R_UpLeg', 'R_Thigh'],
...
export const MIXAMO_VARIATIONS: Record<string, string[]> = {
  ...
  'DEF-shoulderR': ['DEF-shoulderR', 'mixamorig:RightShoulder', 'RightShoulder'],
...
export function createBoneMapping(
  ...
  for (const sourceName of sourceBoneNames) {
    ...
    if (mappedName) {
      ...
      const actualTargetName = targetBoneNames.find(name =>
      ...
      if (actualTargetName) {
        ...
        unmappedCount++
        console.log(`  ❌ ${sourceName} → ${mappedName} (target bone not found)`)
      }
    } else {
      unmappedCount++
  ...
  console.log(`✅ Mapping complete: ${mappedCount} mapped, ${unmappedCount} unmapped`)
  console.log(`   Mapping quality: ${(mappedCount / sourceBoneNames.length * 100).toFixed(1)}%`)
