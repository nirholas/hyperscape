...
export class DistanceChildTargetingSolver extends AutoSkinSolver {
  ...
  calculateWeights(): { skinIndices: number[], skinWeights: number[] } {
    ...
    this.cachedMedianPositions = this.bones.map(bone => this.getMedianBonePosition(bone))
    ...
    for (let i = 0; i < vertexCount; i++) {
      ...
      this.bones.forEach((bone, boneIndex) => {
        const distance = vertexPos.distanceTo(this.cachedMedianPositions[boneIndex])
  ...
  private reassignToParentBones(
    ...
    for (let i = 0; i < vertexCount; i++) {
      ...
      const dirToCurrent = new THREE.Vector3().subVectors(vertexPos, currentBonePos).normalize()
      const dirToParent = new THREE.Vector3().subVectors(vertexPos, parentBonePos).normalize()
