...
import { createBoneMapping, MESHY_TO_MIXAMO, MIXAMO_TO_MESHY } from './BoneMappings'
...
export class AnimationRetargeter {
  ...
  constructor(
    ...
    this.boneMapping = createBoneMapping(sourceBoneNames, targetBoneNames, MIXAMO_TO_MESHY)

    console.log('🎬 AnimationRetargeter initialized')
    console.log('  Source skeleton:', sourceSkeleton.bones.length, 'bones (animation rig)')
    console.log('  Target skeleton:', targetSkeleton.bones.length, 'bones (character rig)')
  ...
  private calculateRestPoseOffsets(): void {
    ...
    Array.from(this.boneMapping.entries()).forEach(([sourceBoneName, targetBoneName]) => {
      ...
      if (sourceBone && targetBone) {
        ...
        const offset = targetLocalQuat.clone().invert().multiply(sourceLocalQuat)
        ...
        if (sourceLength > 0.001) {
          this.boneLengthRatios.set(sourceBoneName, targetLength / sourceLength)
        } else {
  ...
  retargetAll(): RetargetedAnimation[] {
    ...
    console.log(`✅ Retargeted ${retargeted.length}/${this.sourceAnimations.length} animations`)
    return retargeted
  }
  ...
  retargetClip(sourceClip: THREE.AnimationClip): RetargetedAnimation | null {
    ...
    console.log(`  ✅ Mapped ${mappedTrackCount} tracks, skipped ${skippedTrackCount}`)
   ...
   */
  private applyQuaternionOffset(values: ArrayLike<number>, offset: THREE.Quaternion): Float32Array {
   ...
   */
  private scalePositionValues(values: ArrayLike<number>, ratio: number): Float32Array {
    const newValues = new Float32Array(values.length)
