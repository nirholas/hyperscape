...
export function retargetAnimation(
  ...
  console.log('[AnimationRetargeting] Using rootToHips for scaling:', rootToHips)
  ...
  clip.tracks.forEach(track => {
    ...
    if (!mixamoRigNode || !mixamoRigNode.parent) {
      console.warn(`[AnimationRetargeting] Mixamo rig node not found: ${mixamoRigName}`)
      return
    }
  ...
  console.log('[AnimationRetargeting] Retargeted tracks:', retargetedTracks.length)
  console.log('[AnimationRetargeting] Retargeted track names:', retargetedTracks.map(t => t.name))

  // Debug: Log sample values from first quaternion track
  const firstQuatTrack = retargetedTracks.find(t => t instanceof THREE.QuaternionKeyframeTrack)
  if (firstQuatTrack && firstQuatTrack instanceof THREE.QuaternionKeyframeTrack) {
    const firstValues = firstQuatTrack.values.slice(0, 4)
    console.log('[AnimationRetargeting] Sample quaternion values (first keyframe):')
    console.log(`  Track: ${firstQuatTrack.name}`)
    console.log(`  Values: [${firstValues.map(v => v.toFixed(3)).join(', ')}]`)
  }
