  ...
  segmentFingers(
    ...
    for (let y = 0; y < imageHeight; y++) {
      for (let x = 0; x < imageWidth; x++) {
        const pixel = { x, y };
        const nearestFinger = this.findNearestFinger(
          pixel,
          fingerSeeds,
        ...
        if (nearestFinger) {
          ...
          // Update bounds
          masks[nearestFinger].bounds.minX = Math.min(
            masks[nearestFinger].bounds.minX,
            x,
          );
          masks[nearestFinger].bounds.maxX = Math.max(
            masks[nearestFinger].bounds.maxX,
            x,
          );
          masks[nearestFinger].bounds.minY = Math.min(
            masks[nearestFinger].bounds.minY,
            y,
          );
          masks[nearestFinger].bounds.maxY = Math.max(
            masks[nearestFinger].bounds.maxY,
            y,
          );
        }
      }
    ...
    fingers.forEach((finger) => {
      masks[finger].data = this.cleanupMask(
        masks[finger].data,
        imageWidth,
        imageHeight,
      );
  ...
  segmentMeshVertices(
    ...
    for (let i = 0; i < positions.count; i++) {
      ...
      const pixelX = Math.floor(
        (ndcX + 1) * 0.5 * fingerSegmentation.palm.width,
      );
      const pixelY = Math.floor(
        (1 - ndcY) * 0.5 * fingerSegmentation.palm.height,
      );

      // Check which finger mask contains this pixel
      if (
        pixelX >= 0 &&
        pixelX < fingerSegmentation.palm.width &&
        pixelY >= 0 &&
        pixelY < fingerSegmentation.palm.height
      ) {
