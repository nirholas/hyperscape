...
export class AssetNormalizationService {
  ...
  async validateNormalization(
    ...
    if (assetType === "character") {
      ...
      if (!originCorrect) {
        errors.push(
          `Grip not at origin. Expected grip Y: ${expectedGripY.toFixed(3)}`,
        );
      }
    } else {
      ...
      if (!originCorrect) {
        errors.push(
          `Center not at origin. Center: (${center.x.toFixed(3)}, ${center.y.toFixed(3)}, ${center.z.toFixed(3)})`,
        );
      }
    }
    ...
    return {
      ...
      normalized:
        originCorrect && orientationCorrect && scaleCorrect && !hasTransforms,
