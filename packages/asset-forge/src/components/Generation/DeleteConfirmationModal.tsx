...
> = ({ showDeleteConfirm, materialPresets, onClose, onConfirm }) => {
      ...
      <ModalBody>
        <div className="space-y-4">
          <p className="text-text-secondary">
            Are you sure you want to delete this material preset? This action
            cannot be undone.
          </p>
          <div className="flex items-center gap-3 p-4 bg-error bg-opacity-10 rounded-lg">
            <AlertCircle className="w-5 h-5 text-error flex-shrink-0" />
            <p className="text-sm">
              Material preset <strong>{preset?.displayName}</strong> will be
              permanently deleted.
            </p>
          </div>
        </div>
