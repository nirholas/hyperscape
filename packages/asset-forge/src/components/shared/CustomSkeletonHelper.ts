...
export class CustomSkeletonHelper extends THREE.LineSegments {
  ...
  private readonly jointTexture = new THREE.TextureLoader().load(
    "/images/skeleton-joint-point.png",
  );

  constructor(
    rootBone: THREE.Bone,
    options: { linewidth?: number; color?: number; jointColor?: number } = {},
  ) {
    ...
    const pointPositions = new THREE.Float32BufferAttribute(
      bones.length * 3,
      3,
    );
  ...
  dispose(): void {
    ...
    if (this.joint_points.material instanceof THREE.Material)
