...
export function BoneMappingInterface({
  ...
  function buildBoneTree(bones: THREE.Bone[]): BoneNode[] {
    ...
    bones.forEach(bone => {
      ...
      if (bone.parent && bone.parent instanceof THREE.Bone && boneMap.has(bone.parent)) {
  ...
  function handleAutoMap() {
    ...
    sourceSkeleton.bones.forEach(sourceBone => {
      ...
      const exactMatch = targetSkeleton.bones.find(b => b.name.toLowerCase() === sourceName)
      if (exactMatch) {
  ...
  function handleSavePreset() {
    ...
    const presets = JSON.parse(localStorage.getItem('boneMappingPresets') || '{}')
  ...
  function renderBoneTree(nodes: BoneNode[], type: 'source' | 'target'): React.ReactElement[] {
    ...
    function renderNode(node: BoneNode) {
      ...
      const isTargetOfMapping = type === 'target' && Object.values(mapping).includes(node.name)
          ...
          <div className="flex items-center justify-between">
            ...
            {type === 'source' && mapping[node.name] && (
              <div className="flex items-center gap-2">
                <span className="text-xs text-success">→ {mapping[node.name]}</span>
                <button
                  onClick={(e) => {
                    e.stopPropagation()
  ...
  return (
    <div className="flex flex-col h-full bg-bg-secondary rounded-lg shadow-lg border border-border-primary">
      ...
      <div className="p-4 border-b border-border-primary bg-bg-tertiary">
        <h2 className="text-xl font-bold mb-2 text-text-primary">Visual Bone Mapping</h2>

        {/* Usage Instructions */}
        <div className="mb-4 p-3 bg-info/10 border border-info/20 rounded">
          <p className="text-sm font-semibold text-text-primary mb-2">How to Use:</p>
          <ol className="text-xs text-text-secondary space-y-1 list-decimal pl-4">
            <li>Click a bone in the <strong>Source Skeleton</strong> (left, your Meshy model)</li>
            <li>Click the corresponding bone in the <strong>Target Skeleton</strong> (right, Mixamo rig)</li>
            <li>Click <strong>"Link Selected"</strong> to create the mapping</li>
            <li>Or click <strong>"Auto-Map"</strong> to automatically match bones by name</li>
          ...
          <div className="flex-1 overflow-y-auto bg-bg-primary">
            {targetTree.length > 0 ? (
              ...
              <div className="p-4 text-text-secondary text-center space-y-2">
                ...
                <p className="text-xs text-text-tertiary">
                  Go back to Step 2 (Load Skeleton) and select a target rig (Human/Quadruped/Bird)
                </p>
              </div>
            )}
          </div>
