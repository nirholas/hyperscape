    ...
    try {
      ...
      try {
        ...
        report.issues.missingFiles.push(
          `${assetId}: Missing model file ${modelFile}`,
        );
      }
    } catch (error) {
      ...
      if (isNodeError && (error as NodeJS.ErrnoException).code === "ENOENT") {
        ...
        report.issues.invalidMetadata.push(
          `${assetId}: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
  }
  ...
  for (const [assetId, metadata] of allMetadata.entries()) {
    if (isVariantAsset(metadata)) {
      ...
      if (!allMetadata.has(metadata.parentBaseModel)) {
        report.variants.missingParent.push(
          `${assetId} -> ${metadata.parentBaseModel}`,
        );
      }
    }
    ...
    if (isBaseAsset(metadata)) {
      ...
      for (const variantId of metadata.variants || []) {
        if (!allMetadata.has(variantId)) {
          report.variants.orphaned.push(
            `${assetId} lists non-existent variant: ${variantId}`,
          );
        }
      }
    }
    ...
    if (metadata.isBaseModel && metadata.isVariant) {
      report.issues.inconsistentData.push(
        `${assetId}: Both isBaseModel and isVariant are true`,
      );
    }

    if (!metadata.id || metadata.id !== assetId) {
      report.issues.inconsistentData.push(
        `${assetId}: ID mismatch (${metadata.id})`,
      );
    }
  }
...
async function printReport(report: AuditReport) {
  ...
  console.log(
    chalk.green(`  ✓ With meshyTaskId: ${report.baseModels.withMeshyTaskId}`),
  );
  console.log(
    chalk.red(
      `  ✗ Without meshyTaskId: ${report.baseModels.withoutMeshyTaskId.length}`,
    ),
  );
  console.log(
    chalk.white(`  With variants: ${report.baseModels.withVariants}`),
  );
