 ...
 */
async function buildTree(fullPath: string, relativePath: string): Promise<AssetTree> {
 ...
 */
export async function getAssetsByCategory(category: AssetCategory): Promise<AssetFile[]> {
...
export async function getAssetFile(relativePath: string): Promise<{
  ...
  try {
    ...
    // Binary files (models, audio, images)
 ...
 */
export async function getAssetMetadata(assetPath: string): Promise<AssetMetadata | null> {
  try {
 ...
 */
export async function getAllAssetsWithMetadata(): Promise<Array<{
  path: string;
  metadata: AssetMetadata;
}>> {
 ...
 */
export async function getManifestData(manifestFile: ManifestFile): Promise<unknown[]> {
  try {
 ...
 */
export async function updateManifest(manifestFile: ManifestFile, data: unknown[]): Promise<void> {
  try {
  ...
  try {
    ...
    for (const manifest of manifests) {
      for (const item of manifest.data) {
        ...
        // Search all string fields for asset references
        for (const [key, value] of Object.entries(record)) {
          if (typeof value === 'string' && value.includes(assetPath)) {
            references.push({
  ...
  try {
    for (const category of ASSET_CATEGORIES) {
      ...
      if (category === 'manifests') {
        stats.manifestCount = assets.filter(a => a.extension === '.json').length;
      }
    }
