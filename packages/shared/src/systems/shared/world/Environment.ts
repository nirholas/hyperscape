...
export class Environment extends System {
  ...
  async updateModel() {
    ...
    let glb: LoaderResult | undefined;
    try {
      glb = this.world.loader?.get("model", url);
      if (!glb)
      ...
      console.warn(
        `[Environment] Failed to load environment model from ${url}:`,
        error instanceof Error ? error.message : error,
      );
      // Continue without environment model - game can still run
      this.model = null;
      return;
    }
    if (!glb) return;
