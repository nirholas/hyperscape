...
export class ServerBot extends System {
  ...
  override start(): void {
    ...
    if (process.env.DISABLE_BOTS === "true") {
      Logger.info(
        "[ServerBot] Bots disabled via DISABLE_BOTS environment variable",
      );
      return;
    }
  ...
  private initializeBehaviors(): void {
    this.behaviors = [
      ...
      },
      {
        ...
        weight: 4,
        canExecute: () =>
          this.hasNearbyEntities() && Date.now() >= this.dwellUntil,
  ...
  private behaviorLoop(): void {
    ...
    const now = Date.now();
    const availableBehaviors = this.behaviors.filter(
      (b) => b.canExecute() && now - b.lastExecuted > b.cooldown,
    );

    if (availableBehaviors.length > 0) {
      // Weighted random selection
      const totalWeight = availableBehaviors.reduce(
        (sum, b) => sum + b.weight,
        0,
      );
  ...
  private getClientPlayerPosition(): THREE.Vector3 {
    ...
    if ("node" in player && player.node && player.node.position) {
      ...
      if (this._tempVec3.y < -20 && this._tempVec3.y > -22) {
        ...
        throw new Error(
          `ServerBot detected corrupted position Y=${this._tempVec3.y} - likely packet encoding bug!`,
        );
      }
      ...
      if (this._tempVec3.y < -100 || this._tempVec3.y > 500) {
        Logger.warn(
          `[ServerBot] Unusual Y position detected: ${this._tempVec3.y}`,
        );
        this.stats.errors++;
      }
