...
export class WaterSystem {
  ...
  async init(): Promise<void> {
    ...
    if (!this.waterNormal) {
      ...
      try {
        this.waterNormal = await new Promise<THREE.Texture>((resolve, reject) => {
          loader.load(
            url,
        ...
        console.warn('[WaterSystem] Failed to load water normal texture:', error)
   ...
   */
  generateWaterMesh(tile: TerrainTile, waterThreshold: number, tileSize: number): THREE.Mesh {
    ...
    ;(waterMaterial.userData as { _waterUniforms?: unknown })._waterUniforms = waterUniforms
  ...
  update(deltaTime: number, waterMeshes: THREE.Mesh[]): void {
    ...
    for (const waterMesh of waterMeshes) {
      const mat = waterMesh.material as THREE.Material & { userData?: Record<string, unknown> }
      const uniforms = mat?.userData && (mat.userData as { _waterUniforms?: Record<string, { value: unknown }> })._waterUniforms
