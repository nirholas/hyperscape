...
export class CSMHelper extends Group {
   ...
   */
  public displayFrustum: boolean = true;

  /**
   * Whether to display the cascade planes or not.
   */
  public displayPlanes: boolean = true;

  /**
   * Whether to display the shadow bounds or not.
   */
  public displayShadowBounds: boolean = true;

  public frustumLines: LineSegments;
  public cascadeLines: Box3Helper[] = [];
  public cascadePlanes: Mesh[] = [];
  public shadowLines: Group[] = [];

  /**
   * Constructs a new CSM helper.
   *
   * @param csm - The CSM instance to visualize.
   */
  constructor(csm: CSM) {
    super();
  ...
  update(): void {
    ...
    const frustumLines = this.frustumLines;
    const frustumLinePositions = frustumLines.geometry.getAttribute(
      "position",
    ) as BufferAttribute;
    const cascadeLines = this.cascadeLines;
    const cascadePlanes = this.cascadePlanes;
  ...
  dispose(): void {
    ...
    (frustumLines.material as LineBasicMaterial).dispose();
    ...
    for (let i = 0; i < cascades; i++) {
      ...
      (cascadePlane.material as MeshBasicMaterial).dispose();
      shadowLine.dispose();
    }
  }
}
